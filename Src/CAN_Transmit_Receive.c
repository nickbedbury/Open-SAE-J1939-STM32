...
ENUM_J1939_STATUS_CODES CAN_Send_Message(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, uint32_t ID, uint8_t data[]) {
	...
	if (HAL_OK != HAL_CAN_AddTxMessage(CanHandle, &TxHeader, data, TxMailbox))
	{
 ...
 * PGN: 0x00EA00 (59904)
 */
ENUM_J1939_STATUS_CODES CAN_Send_Request(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, uint32_t ID, uint8_t PGN[]) {
	ENUM_J1939_STATUS_CODES status = STATUS_SEND_OK;
	#if PROCESSOR_CHOICE == STM32
	CAN_TxHeaderTypeDef TxHeader;
	...
	if (HAL_OK != HAL_CAN_AddTxMessage(CanHandle, &TxHeader, PGN, TxMailbox))
	{
		status = STATUS_SEND_ERROR;
	}
	#elif PROCESSOR_CHOICE == ARDUINO
...
bool CAN_Read_Message(CAN_HandleTypeDef *CanHandle, uint32_t *ID, uint8_t data[]) {
