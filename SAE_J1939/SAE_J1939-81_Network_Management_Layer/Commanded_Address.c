...
ENUM_J1939_STATUS_CODES SAE_J1939_Send_Commanded_Address(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, J1939 *j1939, uint8_t DA, uint8_t new_ECU_address, uint32_t identity_number, uint16_t manufacturer_code, uint8_t function_instance, uint8_t ECU_instance, uint8_t function, uint8_t vehicle_system, uint8_t arbitrary_address_capable, uint8_t industry_group, uint8_t vehicle_system_instance) {
	...
	ENUM_J1939_STATUS_CODES status = SAE_J1939_Send_Transport_Protocol_Connection_Management(CanHandle, TxMailbox, j1939, DA);
	...
	if(j1939->this_ecu_tp_cm.control_byte == CONTROL_BYTE_TP_CM_BAM)
		return SAE_J1939_Send_Transport_Protocol_Data_Transfer(CanHandle, TxMailbox, j1939, DA);
	return status;
...
void SAE_J1939_Read_Commanded_Address(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, J1939 *j1939, uint8_t data[]) {
	...
	SAE_J1939_Send_Address_Delete(CanHandle, TxMailbox, j1939, 0xFF, j1939->information_this_ECU.this_ECU_address);
