...
ENUM_J1939_STATUS_CODES SAE_J1939_Send_Request_DM14(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, J1939 *j1939, uint8_t DA, uint16_t number_of_requested_bytes, uint8_t pointer_type, uint8_t command, uint32_t pointer, uint8_t pointer_extension, uint16_t key) {
	...
	data[6] = key;
	data[7] = key >> 8;
	return CAN_Send_Message(CanHandle, TxMailbox, ID, data);
}
...
ENUM_J1939_STATUS_CODES SAE_J1939_Read_Request_DM14(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, J1939 *j1939, uint8_t DA, uint8_t data[]) {
	...
	/* Here we ask for flash, eeprom or ram memory and use pointers */
	...
	uint16_t seed = key;
	status = SAE_J1939_Send_Response_DM15(CanHandle, TxMailbox, j1939, DA, number_of_allowed_bytes, status, EDC_parameter, EDCP_extention, seed);

	/* Check if our message was OK - Send DM16 binary data transfer */
	if(status == STATUS_DM15_PROCEED) {
		if(SAE_J1939_Send_Binary_Data_Transfer_DM16(CanHandle, TxMailbox, j1939, DA, number_of_occurences, raw_binary_data) == STATUS_SEND_OK)
			status = SAE_J1939_Send_Response_DM15(CanHandle, TxMailbox, j1939, DA, number_of_allowed_bytes, STATUS_DM15_OPERATION_COMPLETED, EDC_parameter, EDCP_extention, seed);
		else
			status = SAE_J1939_Send_Response_DM15(CanHandle, TxMailbox, j1939, DA, number_of_allowed_bytes, STATUS_DM15_OPERATION_FAILED, EDC_parameter, EDCP_extention, seed);
	}
	return status;
