...
void SAE_J1939_Read_Transport_Protocol_Connection_Management(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	if(j1939->from_other_ecu_tp_cm.control_byte == CONTROL_BYTE_TP_CM_RTS){
		...
		SAE_J1939_Send_Transport_Protocol_Connection_Management(CanHandle, TxMailbox, j1939, SA);
	}
	...
	if(j1939->from_other_ecu_tp_cm.control_byte == CONTROL_BYTE_TP_CM_CTS)
		SAE_J1939_Send_Transport_Protocol_Data_Transfer(CanHandle, TxMailbox, j1939, SA);
}
