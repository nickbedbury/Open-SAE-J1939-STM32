...
void SAE_J1939_Read_Transport_Protocol_Data_Transfer(CAN_HandleTypeDef *CanHandle, uint32_t *TxMailbox, J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	if(j1939->from_other_ecu_tp_cm.control_byte == CONTROL_BYTE_TP_CM_RTS)
		SAE_J1939_Send_Acknowledgement(CanHandle, TxMailbox, j1939, SA, CONTROL_BYTE_TP_CM_EndOfMsgACK, GROUP_FUNCTION_VALUE_NORMAL, PGN);

	/* Check what type of function that message want this ECU to do */
	switch (PGN) {
	case PGN_COMMANDED_ADDRESS:
		SAE_J1939_Read_Commanded_Address(CanHandle, TxMailbox, j1939, complete_data);								/* Insert new name and new address to this ECU */
		break;
	case PGN_DM1:
